...
async def get_teacher_schedule(client: GroupScheduleRestClient, from_date: str, to_date: str, teacher: str, **kwargs):
    with external_call(client) as rest_client:
        data, status = await rest_client.schedule(from_date=from_date, to_date=to_date, value=teacher)
    return parse(data), status
